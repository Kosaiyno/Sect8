export async function ensure0GChain(): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    try {
      ...
      if (switchError && switchError.code === 4902) {
        try {
          ...
          return { success: false, error: String(addError?.message || addError) };
        }
      }
      return { success: false, error: String(switchError?.message || switchError) };
    }
  } catch (e: any) {
