...
export default function MarketPage() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-10 px-6 py-10 animate-fade-in">
      {/* ─── HEADER SECTION ─── */}
      <section className="fintech-card p-12 md:p-16">
        <div className="flex flex-col gap-10 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-4xl space-y-8">
            ...
            <h1 className="font-outfit text-4xl font-black tracking-[-0.05em] text-[#0f1629] md:text-7xl md:leading-[1.1]">
            ...
            {dataSource === 'rentcast-cache' && (
              <div className="inline-flex items-center gap-3 rounded-2xl border border-[#b8942f]/20 bg-[#b8942f]/05 px-6 py-4 text-base font-bold text-[#b8942f]">
          ...
          <div className="fintech-card w-full max-w-md p-10 shadow-2xl xl:min-w-[450px] border-2 border-gray-100">
            <div className="mb-8 flex items-center gap-2.5 text-[11px] font-black uppercase tracking-[0.26em] text-[#b8942f]">
              <Search size={14} />
            ...
            <div className="space-y-8">
              ...
              <button 
                ...
                className="btn-primary flex w-full items-center justify-center gap-4 py-5 text-lg shadow-[0_12px_24px_rgba(184,148,47,0.2)] disabled:opacity-50"
              >
      ...
      {/* ─── MARKET QUEUE ─── */}
      <div className="fintech-card p-10 md:p-16 flex flex-col md:flex-row md:items-center justify-between gap-10 border-b-[8px] border-b-[#b8942f]/20">
        <div className="space-y-3">
          <div className="platform-eyebrow text-sm">Market queue</div>
          <div className="font-outfit text-4xl md:text-5xl font-black text-[#0f1629]">
            I found {properties.length} homes in <span className="text-[#b8942f]">{selectedZip || "ZIP Market"}</span>
          </div>
        </div>
        <div className="max-w-md text-lg font-bold leading-relaxed text-[#64748b]">
      ...
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
        {properties.map((property, idx) => (
          <div key={property.id} className="fintech-card p-12 group transition-all duration-500 hover-lift border-2 border-gray-100 hover:border-[#b8942f]/40">
      ...
      {!isLoading && properties.length === 0 && (
        <div className="fintech-card p-20 text-center flex flex-col items-center justify-center space-y-6">
          <div className="h-20 w-20 rounded-full bg-[#f8f9fb] flex items-center justify-center mb-2">
            <Search size={32} className="text-[#b8942f]" />
          </div>
          <div className="font-outfit text-4xl font-black text-[#0f1629]">Market snapshot unavailable</div>
