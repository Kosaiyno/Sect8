...
export async function POST(req) {
    try {
        ...
        if (!zip)
            return NextResponse.json({ success: false, error: 'zipCode required' }, { status: 400 });
        // Try RentCast
        ...
        if (rc && Array.isArray((rc === null || rc === void 0 ? void 0 : rc.data) || rc)) {
            ...
            listings = items.map((it) => {
                ...
                return ({
                    ...
                    image: it.image || it.photo || ((_b = it.photos) === null || _b === void 0 ? void 0 : _b[0]) || `https://via.placeholder.com/560x420?text=${encodeURIComponent((it.address || it.title || 'Listing').slice(0, 24))}`
                });
            });
        }
        else {
