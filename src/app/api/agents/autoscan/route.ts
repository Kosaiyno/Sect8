...
export async function POST(req: Request) {
  try {
    ...
    for (const [id, agent] of Object.entries<any>(targetAgents || {})) {
      if (!agent || !agent.preferences) continue;
      ...
      for (const p of listings) {
        ...
        if (isBanger) {
          ...
          try {
            ...
            if (upload?.success) logs.push({ time: Date.now(), type: 'storage', message: `Saved memory for ${rec.id} -> ${upload.hash}`, hash: upload.hash });
            else logs.push({ time: Date.now(), type: 'storage_error', message: String(upload?.error || 'unknown') });
          } catch (e:any) {
