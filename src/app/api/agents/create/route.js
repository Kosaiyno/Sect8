...
export async function POST(req) {
    try {
        ...
        if (!storageRes.success)
            return NextResponse.json({ success: false, error: storageRes.error }, { status: 500 });
        ...
        if (!tokenId) {
            if (!storageRes.hash)
                return NextResponse.json({ success: false, error: 'storage hash missing' }, { status: 500 });
            const identityRes = await registerAgentIdentity(owner, storageRes.hash);
            if (!identityRes.success)
                return NextResponse.json({ success: false, error: identityRes.error }, { status: 500 });
