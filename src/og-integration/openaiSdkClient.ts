export async function tryOpenAISdkCall(baseUrl: string, headers: Record<string,string>, payload: any) {
  try {
    ...
    let apiKey = headers?.Authorization || headers?.authorization || '';
    ...
    const messages = payload.messages || payload?.messages || (Array.isArray(payload) ? payload : undefined);
